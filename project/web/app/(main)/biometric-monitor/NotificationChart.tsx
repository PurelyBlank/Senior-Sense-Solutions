...
export default function NotificationChart() {
  ...
  const clearHeartRateAlert = (timestamp: string, patient_name: string) => {
    if (!heartRateData) return;
    const updatedData = heartRateData.filter(
      entry => !(entry.timestamp === timestamp && entry.patient_name === patient_name)
    );
    setHeartRateData(updatedData.length > 0 ? updatedData : null);
  };

  const clearFallAlert = (timestamp: string, patient_name: string) => {
    if (!fallData) return;
    const updatedData = fallData.filter(
      entry => !(entry.timestamp === timestamp && entry.patient_name === patient_name)
    );
    setFallData(updatedData.length > 0 ? updatedData : null);
  };
    ...
    <div className="notifications-box">
      ...
      <div className="notifications-details">
        {heartRateAlerts.map((alert, idx) => (
          <div key={idx} className="notifications-row-hoverable">
            ...
            <button
              className="clear"
              onClick={() => clearHeartRateAlert(alert!.timestamp, alert!.patient_name)}
              aria-label="Clear Heart Rate Alert"
            >
              Clear
            </button>
          </div>
        ...
        {fallData && fallData.length > 0 && fallData.map((fall, index) => (
          <div key={index} className="notifications-row-hoverable">
            <CgDanger size={20} />
            <div className="notifications-text">
            ...
            <button
              className="clear"
              onClick={() => clearFallAlert(fall.timestamp, fall.patient_name)}
              aria-label="Clear Fall Alert"
            >
              Clear
            </button>
          </div>
